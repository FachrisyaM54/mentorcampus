...
<head>
    ...
    <title>Sign Up - MentorCampus</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="antialiased">

<!-- py-10 md:py-20 memastikan halaman register yang panjang ini tetap aman dari potong atas bawah -->
<div class="min-h-screen flex items-center justify-center px-4 py-10 md:py-20 bg-cover bg-center relative"
    ...
    <div class="relative z-10 w-full max-w-[380px] my-auto">
        <div class="bg-white/10 backdrop-blur-xl rounded-lg shadow-2xl p-6 border border-white/20">

            {{-- Header --}}
            <div class="text-center mb-5">
                <img src="{{ asset('storage/profile/logo.png') }}" class="h-10 mx-auto mb-3" alt="Logo">
                <h2 class="text-xl font-bold text-white tracking-tight">Sign Up</h2>
                <p class="text-xs text-gray-300 mt-1">create your new account MentorCampus</p>
            </div>
            ...
            <form method="POST" action="{{ route('register') }}">
                ...
                {{-- NAMA --}}
                <div class="mb-3.5">
                    <label for="nama" class="block text-xs font-semibold text-gray-200 mb-1 uppercase tracking-wider">Full Name</label>
                    <input type="text" name="nama" id="nama" value="{{ old('nama') }}" required
                           class="w-full bg-transparent border border-gray-400 rounded-lg px-3.5 py-2 text-white text-sm outline-none focus:border-[#175BAF] focus:ring-1 focus:ring-[#175BAF] transition-all placeholder-gray-400"
                           placeholder="John Doe">
                </div>

                {{-- EMAIL --}}
                <div class="mb-3.5">
                    <label for="email" class="block text-xs font-semibold text-gray-200 mb-1 uppercase tracking-wider">Email Address</label>
                ...
                {{-- PASSWORD --}}
                <div class="mb-3.5">
                    <label for="password" class="block text-xs font-semibold text-gray-200 mb-1 uppercase tracking-wider">Password</label>
                    <div class="relative">
                ...
                {{-- REPEAT PASSWORD --}}
                <div class="mb-3.5">
                    <label for="password_confirmation" class="block text-xs font-semibold text-gray-200 mb-1 uppercase tracking-wider">Repeat Password</label>
                    <div class="relative">
                        <input id="password_confirmation" type="password" name="password_confirmation" required
                               class="w-full bg-transparent border border-gray-400 rounded-lg px-3.5 py-2 text-white text-sm outline-none focus:border-[#175BAF] focus:ring-1 focus:ring-[#175BAF] transition-all placeholder-gray-400"
                               placeholder="••••••••">
                        <button type="button" onclick="togglePassword('password_confirmation', this)" class="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-300 hover:text-white transition text-sm select-none outline-none">
                            👁️
                        </button>
                    </div>
                </div>
                ...
                {{-- GENDER & SEMESTER --}}
                <div class="mb-4">
                    <label class="block text-xs font-semibold text-gray-200 mb-1 uppercase tracking-wider">Academic Info</label>
                    <div class="grid grid-cols-2 gap-3">
                        <select name="gender" required
                                class="w-full bg-transparent border border-gray-400 rounded-lg px-3 py-2 text-gray-200 text-sm outline-none focus:border-[#175BAF] focus:ring-1 focus:ring-[#175BAF] transition-all [&>option]:bg-gray-800 [&>option]:text-white">
                        ...
                        <select name="semester" required
                                class="w-full bg-transparent border border-gray-400 rounded-lg px-3 py-2 text-gray-200 text-sm outline-none focus:border-[#175BAF] focus:ring-1 focus:ring-[#175BAF] transition-all [&>option]:bg-gray-800 [&>option]:text-white">
                            <option value="" class="text-gray-400">Semester</option>
                            @for($i=1;$i<=14;$i++)
            ...
            <div class="text-center mt-5">
                <p class="text-xs text-gray-300">
                    Do you have account?
                    <a href="{{ route('login') }}" class="text-blue-400 hover:text-blue-300 hover:underline font-bold ml-1">Sign In</a>
                </p>
            </div>
